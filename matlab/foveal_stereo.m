...
figure(1)
subplot(1,2,1); imshow(left_image_gray);
subplot(1,2,2); imshow(right_image_gray);
...
clear foveal
foveal = foveal_interface(N, M, xc, yc, NRINGS, NSECTORS, RMIN, RMAX, interp, sp, full, horizontal_disparities, sigma, q, pole, 1); % Create a class instance
...
marker_idx = (disparity_map_matlab == -realmax('single'));
disparity_map_matlab(marker_idx) = min(disparity_map_matlab(~marker_idx));

disparity_map_matlab = disparity_map_matlab-min_disparity;
